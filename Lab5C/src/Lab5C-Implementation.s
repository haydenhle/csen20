...
/* ------------------------------------------------------------------------------------------------------------------- */
        ...
        .align
MxPlusB:
	    MULS.N R1, R0, R1	    // R1 = dvnd; R2 - dvsr (Assume this product does not overflow)

        SMULL R12, R0, R1, R2	// R0 = dvnd*dvsr       (Sign of product = Sign of x*mtop*mbtm)

        ASRS.N R0, R0, #31	    // R0 = (dvnd*dvsr) >> 31

        MUL R0, R0, R2	    // R0 = (dvnd*dvsr) >> 31) * dvsr

        LSL R0, R0, #1	        // R0 = ((dvnd*dvsr) >> 31) * dvsr) << 1) + dvsr
        ADD R0, R0, R2
